...
class Ui_MainWindow(object):
    def __init__(self):
        ...
        self.cursor.execute(
            "CREATE TABLE IF NOT EXISTS data(queue_names TEXT, scoresEng TEXT, scoresTur TEXT)")
        self.con.commit()
    ...
    def setupUi(self, MainWindow):
        ...
        self.palette.setBrush(QtGui.QPalette.Active,
                              QtGui.QPalette.Window, dark_gray_brush)
        self.palette.setBrush(QtGui.QPalette.Inactive,
                              QtGui.QPalette.Window, dark_gray_brush)
        self.palette.setBrush(QtGui.QPalette.Disabled,
                              QtGui.QPalette.Window, dark_gray_brush)
        ...
        self.palette.setBrush(QtGui.QPalette.Active,
                              QtGui.QPalette.WindowText, light_gray_brush)
        self.palette.setBrush(QtGui.QPalette.Inactive,
                              QtGui.QPalette.WindowText, light_gray_brush)
        self.palette.setBrush(QtGui.QPalette.Disabled,
                              QtGui.QPalette.WindowText, light_gray_brush)
        ...
        self.palette.setBrush(QtGui.QPalette.Active,
                              QtGui.QPalette.Text, light_gray_brush)
        self.palette.setBrush(QtGui.QPalette.Active,
                              QtGui.QPalette.Base, dark_gray_brush)
        self.palette.setBrush(QtGui.QPalette.Active,
                              QtGui.QPalette.Highlight, light_gray_brush)
        self.palette.setBrush(QtGui.QPalette.Inactive,
                              QtGui.QPalette.Text, light_gray_brush)
        self.palette.setBrush(QtGui.QPalette.Inactive,
                              QtGui.QPalette.Base, dark_gray_brush)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.actionYeni_Pencere.setText(
            _translate("MainWindow", "Yeni Pencere"))
        self.actionYeni_Pencere.setShortcut(
    ...
    def console_text(self, text, assets=True):
        if assets:
            self.label_2.setText(str(self.label_2.text()) + os.getcwd() +
    ...
    def color(self, R, G, B):
        ...
        self.palette.setBrush(QtGui.QPalette.Active,
                              QtGui.QPalette.Light, light_brush)
        self.palette.setBrush(QtGui.QPalette.Inactive,
                              QtGui.QPalette.Light, light_brush)
        self.palette.setBrush(QtGui.QPalette.Active,
                              QtGui.QPalette.Dark, dark_brush)
        self.palette.setBrush(QtGui.QPalette.Inactive,
                              QtGui.QPalette.Dark, dark_brush)
        self.palette.setBrush(QtGui.QPalette.Active,
                              QtGui.QPalette.Mid, mid_brush)
        self.palette.setBrush(QtGui.QPalette.Inactive,
                              QtGui.QPalette.Mid, mid_brush)
    ...
    def addWords(self):
        ...
        if self.pushButton.text() != "Ekle" and self.label.text() != "Ekle? (e/h)":
            ...
            self.console_text(str(25 * " ") + str(100 * "-") + "\n" + (150 -
                              len(satır_sayısı_yazı)) * " " + satır_sayısı_yazı, assets=False)
    ...
    def studyWords(self, language):
        ...
        try:
            self.rastgele_sayı = random.randrange(
                start=0, stop=len(self.label_list))
            ...
            if self.language == "İngilizce":
                self.label.setText(
                    english_word[random.randint(0, len(english_word) - 1)])
            else:
                self.label.setText(
                    turkish_word[random.randint(0, len(turkish_word) - 1)])
        except:
            self.console_text(
                "Çalışılacak Kelimeler.txt'yi okumada başarısız olundu.\nŞunlar sebep olabilir;\n -Dosya boş olabilir.\n -Dosya kaydedilmemiş olabilir.")
        ...
        if self.pushButton.text() == "Kontrol Et":
            ...
            if self.language == "İngilizce":
                if user_text in self.lineEdit_list_turkish[self.rastgele_sayı]:
                    a = self.lineEdit_list_turkish[self.rastgele_sayı].index(
                        user_text)
                    self.console_text("d0ru Cevap")
                    if len(self.lineEdit_list_turkish[self.rastgele_sayı]) >= 2:
                        self.console_text("Diğer anlamları; " + str(
                            self.lineEdit_list_turkish[self.rastgele_sayı][0:a] + self.lineEdit_list_turkish[self.rastgele_sayı][a + 1:]))
                    self.lineEdit.clear()
                ...
                if user_text in self.lineEdit_list_english[self.rastgele_sayı]:
                    a = self.lineEdit_list_english[self.rastgele_sayı].index(
                        user_text)
                    self.console_text("d0ru Cevap")
                    if len(self.lineEdit_list_english[self.rastgele_sayı]) >= 2:
                        self.console_text("Diğer anlamları; " + str(
                            self.lineEdit_list_english[self.rastgele_sayı][0:a] + self.lineEdit_list_english[self.rastgele_sayı][a + 1:]))
                    self.lineEdit.clear()
                    self.studyWords(self.language)
            ...
            if self.label.text() == "İngilizcesi:":
                if len(self.lineEdit.text()) in range(1, 76):
                    ...
                    self.console_text("İngilizce kelime: " +
                                      self.lineEdit.text())
                    
                    self.lineEdit.clear()
                elif len(self.lineEdit.text()) > 75:
                    self.console_text(
                        "Yüksek karakter içeriği, lütfen İngilizce kelimenizin en fazla 75 karakter uzunluğunda olmasına dikkat edin.")
                ...
                if len(self.lineEdit.text()) in range(1, 76):
                    ...
                    self.console_text(
                        "Yüksek karakter içeriği, lütfen Türkçe kelimenizin en fazla 75 karakter uzunluğunda olmasına dikkat edin.")
        else:
            if self.lineEdit.text().lower().rstrip(" ") == "baban kim?":
                self.lineEdit.clear()
                a = int(str(datetime.now())[
                        str(datetime.now()).rfind(".") + 1:])
                ...
                subprocess.Popen("{}\\{}".format(os.getcwd(), self.lineEdit.text().rstrip(
                    " aç")), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE)
                self.lineEdit.clear()
                ...
                lineEdit_color = list(self.lineEdit.text().lower().lstrip(
                    "ard ").replace(" ", "").split(","))
                self.color(int(lineEdit_color[0]), int(
                    lineEdit_color[1]), int(lineEdit_color[2]))
    ...
    def console(self):
        ...
        if figMemory["ing"] and figMemory["tur"]:
            ...
            queueName = str(
                self.lineEdit_list_english[-1][0] + ":" + self.lineEdit_list_turkish[-1][-1][-3:])
            queueName = queueName[:3] + "_" + queueName[-3:]
            queueScore_Eng = float("{:.2f}".format(
                figMemory["ing"].count(True) / len(figMemory["ing"])))
            queueScore_Tur = float("{:.2f}".format(
                figMemory["tur"].count(True) / len(figMemory["tur"])))
            
            try:
                queueScores_Eng = list(
                    str(self.queueData[queueName]["eng"]).lstrip("[").rstrip("]").split(","))
                queueScores_Tur = list(
                    str(self.queueData[queueName]["tur"]).lstrip("[").rstrip("]").split(","))
                for i, j in zip(queueScores_Eng, queueScores_Tur):
                    if queueScores_Eng.index(i) != 0:
                        queueScores_Eng[queueScores_Eng.index(
                            i)], queueScores_Tur[queueScores_Tur.index(j)] = float(i), float(j)
            except KeyError:
                queueScores_Eng = [""]
                queueScores_Tur = [""]
            ...
            try:
                self.queueData[queueName]
                self.cursor.execute("Update data set scoresEng = ?, scoresTur = ? where queue_names = ?", (str(
                    queueScores_Eng), str(queueScores_Tur), queueName))
            except:
                self.cursor.execute("Insert into data Values(?, ?, ?)", (queueName, str(
                    queueScores_Eng), str(queueScores_Tur)))
            ...
            if len(queueScores_Eng) > 1:
                ...
                boolen_values_eng, boolen_values_tur = numpy.array(
                    queueScores_Eng), numpy.array(queueScores_Tur)
                values_len = numpy.array(range(len(queueScores_Eng)))
                graphic.plot(values_len, boolen_values_eng,
                             "blue", marker="o", label="İngilizce")
                graphic.plot(values_len, boolen_values_tur,
                             "red", marker="o", label="Türkçe")
                graphic.legend()
