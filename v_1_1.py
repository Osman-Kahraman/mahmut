...
class Ui_MainWindow(object):
    ...
    def console(self):
        ...
        if figMemory["ing"] and figMemory["tur"]:
            ...
            try:
                queueScores_Eng = list(str(self.queueData[queueName]["eng"]).lstrip("[").rstrip("]").split(","))

                queueScores_Tur = list(str(self.queueData[queueName]["tur"]).lstrip("[").rstrip("]").split(","))
            ...
            try:
                ...
                self.cursor.execute("Insert into data Values(?, ?, ?)", (queueName, str(queueScores_Eng), str(queueScores_Tur)))
