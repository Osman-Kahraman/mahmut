...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.lineEdit = DropLineEdit("", self)
...
içe aktar <dosya yolu>: İstenilen kelime grubunu içeri aktarıp diğer kelimeler arasına kaydeder.
...
₺+              """, assets = False
    ...
    def console_text_prepare(self):
        if self.da:
            try:
                self.sev += next(self.da[0]).replace("₺+", int(self.centralwidget.width() * 0.158) * "+").replace("₺-", int(self.centralwidget.width() * 0.1) * "-") + "\n"

                self.label_2.setText(self.sev)
            except StopIteration:
                self.da.remove(self.da[0])
        else:
            self.console_text_memory += self.sev[self.console_text_memory.count("\n") - 1:]
            self.sev = ""
            self.label_2.setText(self.console_text_memory)
            self.timer.stop()
    ...
    def console_text(self, text, assets = True):
        ...
        self.da.append(iter(template.split("\n")))
        self.sev = "\n" * self.console_text_memory.count("\n")
        self.timer.start(1)
    ...
    def study_words(self, language, name):
        try:
            ...
            self.console_text("Çalışılmak istenen bölüm ismi bulunamadı.")
        else:
            self.console_text("{} açılıyor.".format(name))
...
{}
    ...
    v_2_8
₺-
+ Kelime ipucu sistemiyle ilgili v_2_7 ile birlikte gelen hata düzeltildi.
+ Çalışılmak istenen kelime grubu ismi hatalı girildiğinde oluşan program çökme sorunu giderildi.
+ Kelimeleri içe aktarırken yaşanan kelime bozuklukları düzeltildi.
₺-
17.10.2021 Pazar 16:48
₺+                  """, assets = False
                    )
                    ...
                    try:
                        ...
                        if PATH:
                            ...
                            if _type == "Yazı dosyası (*.txt)":
                                ...
                                data.update({"file_upload_PATH": PATH_ + ".txt"})
                                ...
                                data.update({"file_upload_PATH": PATH_ + ".png"})
...
class DropLineEdit(QtWidgets.QLineEdit):
    ...
    def dropEvent(self, event):
        ...
        self.setText("içe aktar {}".format(text.lstrip("file:///")))
        event.acceptProposedAction()
