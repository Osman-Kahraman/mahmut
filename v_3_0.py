...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.command_names = {
            "kd": "Bulunulan dosya konumunu değiştirir.", 
            "aç": "Kelime grubunu açar.",
            "versiyon": "Kullanılan sürümle ilgili bilgi verir.", 
            "güncelle": "İstenilen versiyona günceller.", 
            "dışa aktar": "İstenilen kelime grubunu dışarıya aktarır.", 
            "içe aktar": "İstenilen kelime grubunu içeri aktarıp diğer kelimeler arasına kaydeder.", 
            "dir": "Bütün kelime grup isimlerini gösterir.", 
            "cls": "Geçmişi temizler.", 
            "yeni pencere": "Yeni pencere açar.", 
            "ayarlar": "Ayarları açar.", 
            "kelime düzelt": "Kaydedilmiş kelimeyi düzeltmeyi sağlar.", 
            "kelime bul": "Kelime grubundan kelimeyi bulur.", 
            "kelime ekle": "Kelime ekler.", 
            "ingilizce kelime çalış": "İngilizce kelime çalışmanı sağlar.", 
            "türkçe kelime çalış": "Türkçe kelime çalışmanı sağlar.", 
            "komutlar": "Komutları gösterir.", 
            "hakkında": "Mahmut hakkında bilgi verir.", 
            "çıkış": "Mahmut'u kapatır."
                 }
        ...
        self.action_ngilizce.triggered.connect(lambda: self.word_study_find('İngilizce'))
        self.actionT_rk_e.triggered.connect(lambda: self.word_study_find('Türkçe'))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.actionD_zelt.setText(_translate('MainWindow', 'Düzelt'))
        self.actionD_zelt.setShortcut(_translate('MainWindow', 'Ctrl+Shift+E'))
        self.actionBul.setText(_translate('MainWindow', 'Bul'))
        ...
        self.lineEdit.setPlaceholderText("Komutlarını buraya yazabilirsin")
    ...
    def word_study_find(self, language: str):
        self.lineEdit.clear()
        self.line_color = (0, 0, 127) if language == 'İngilizce' else (170, 0, 0)
        ...
        if not (self.fig_memory['tur'] or self.fig_memory['eng']):
            try:
                ...
                if os.path.isfile(self.data['file_upload_PATH']):
                    ...
                    ret = qm.question(self, 'Halihazırda Bir Kelime Grubu Bulundu', '{} ile mi çalışacaksın?\t\t\t'.format(self.data['file_upload_name']))
                ...
                self.pushButton.setText('{} Başlat'.format(language))
                self.console_write(
                        '{} kelime çalışmaya başlamak için lütfen çalışmak istediğiniz bölümün ismini giriniz.'.format(language), 
                        assets = False
                        )
        else:
    ...
    Mahmut İngilizce kelime hazineni geliştirmek isteyen ufacık bir açık kaynak kodlu programdır. Mahmut, Osman Kahraman tarafından yapılmıştır. 
Mahmut'la ilgili detaylı bilgi ve kaynak kodları için Mahmut GitHub sayfasına bakabilirsin. (https://github.com/Osman-Kahraman/mahmut)
...
Copyright @Osman-Kahraman''', assets = False
    ...
    def game(self):
        ...
        self.label.setText('Sayı? (1-9) = ')
        self.pushButton.setText('Yerleştir') 
    ...
    def word_edit(self):
        ...
        self.label.setText('İngilizcesi:')
        self.pushButton.setText('Değiştir') 
    ...
    def word_find(self, word: str):
        ...
        if word:
            ...
            if found:
                self.console_write('{} tane kelime grubunda bulundu.\n\nBulunduğu yerler;\n{}'.format(len(found), found), assets = False)
            else:
                self.console_write('Kelime bulunamadı.', assets = False)

            return found
        else:
            self.label.setText('Kelime:')
            self.pushButton.setText('Bul') 

    def word_study(self, language, name):
        try:
            words_to_study = self.words[name]
        except KeyError:
            self.console_write('Çalışılmak istenen bölüm ismi bulunamadı.')
        else:
            if self.pushButton.text().endswith('Başlat'):
    ...
    def click(self):
        ...
        if self.pushButton.text() == 'Kontrol Et':
            ...
            if user_text in correct_words:
                ...
                self.console_write('Yanlış Cevap', assets = False)
        ...
        elif self.pushButton.text() == 'Yerleştir' and self.lineEdit.text():
            ...
            if self.label.text() == 'İngilizcesi:':
                ...
                self.console_write('İngilizce kelime: ' + self.lineEdit.text(), assets = False)
            elif self.label.text() == 'Türkçesi:':
                self.label.setText('İngilizcesi:')

                self.word_memory.append(self.lineEdit.text())

                self.console_write('Türkçe kelime: ' + self.lineEdit.text(), assets = False)

            if len(self.word_memory) == 2:
                self.label.setText('Ekle? (e/h)')
                self.pushButton.setText('↑')
        elif self.pushButton.text() == 'Bul':
            self.word_find(self.lineEdit.text())
        elif self.pushButton.text() == 'Değiştir' and self.lineEdit.text():
            if self.label.text() == "İngilizcesi:":
                self.label.setText('Türkçesi:')
            ...
            if len(self.word_memory) >= 2:
                self.label.setText('Değiştir? (e/h)')
                self.pushButton.setText('↑')
        else:
            self.console_write(self.lineEdit.text())
            if len(self.word_memory) >= 2:
                if self.lineEdit.text().lower().replace(' ', '') == 'e':
                    if self.label.text() == "Değiştir? (e/h)":
                        for i in range(0, len(self.word_memory), 2):
                            ...
                            if found:
                                self.console_write('\nKelime düzeltildi.', assets = False)

                        self.word_edit()
                    elif self.label.text() == 'Ekle? (e/h)':
                        self.word_line += 1
                        ...
                        self.console_write('Kelime eklendi.', assets = False)
                    ...
                    if self.label.text() == "Değiştir? (e/h)":
                        self.console_write('Değiştirmeden vazgeçildi.', assets = False)

                        self.word_edit()
                    elif self.label.text() == "Ekle? (e/h)":
                        self.console_write('Eklemeden vazgeçildi.', assets = False)

                        self.add_words()

                    self.word_memory.clear()
            elif self.lineEdit.text().lower() == 'ingilizce kelime çalış':
                self.word_study_find('İngilizce')
            elif self.lineEdit.text().lower() == 'türkçe kelime çalış':
                self.word_study_find('Türkçe')
            elif self.lineEdit.text().lower() == 'baban kim':
            ...
            elif self.lineEdit.text().lower() == 'kelime düzelt':
                self.word_edit()
            elif self.lineEdit.text().lower() == 'kelime ekle':
                self.add_words()
            elif self.lineEdit.text().lower() == 'cls':
                self.console_text = ''
                self.console_write('', assets = False)
            elif self.lineEdit.text().lower() == 'çıkış':
                self.close()
            elif self.lineEdit.text().lower() == "connect 4":
                self.connect_4 = Connect_4()
                self.game()
            elif self.lineEdit.text().lower() == 'hakkında':
                self.about()
            elif self.lineEdit.text().lower() == 'komutlar':
                self.commands()
            elif self.lineEdit.text().lower() == 'yeni pencere':
                self.new_tab()
            elif self.lineEdit.text().lower() == 'ayarlar':
                self.settings()
            elif self.lineEdit.text().lower() == 'aç':
                self.console_write('Bu komutu kullanabilmek için önüne kelime grubunun ismini yazmanız lazım. (Örn. osm_kah aç)', assets = False)
                ...
                try:
                    ...
                    self.console_write('İsim bulunamadı.', assets = False)
                else:
                    text = ''
                ...
                try:
                    ...
                    self.console_write('Dosya yolu bulunamadı.')
            elif self.lineEdit.text().lower().startswith('versiyon'):
                self.console_write(
                    '''
        v_3_0
    + güncelleme bölümünde internet olmazsa gelen hata düzeltildi
    + konsolda görsel düzeltmeleri
    + içe aktar komutunda dosya yolu verilmemesi sonucu oluşan çökme düzeltildi
    + yeni komutlar eklendi
    + versiyon güncellemesindeyken istenen manuel kapatma otomatiğe çevrildi
    + yeni haber sayfası eklendi, güncellemelerin bilgisi oradan verilecek
    + ayarlar bölümü eklendi
    + yeni pencere açma problemi düzeltildi
    + pencereyi özelleştirme geldi
    + kelimeleri ayıran karakteri özelleştirme eklendi (:, ;, ya da istenilen karakter)
    + mahmut'u tanıtan hakkında bölümü eklendi
    + data'daki kelimeleri düzeltmeni sağlaya sistem eklendi
    + yeni tema sayesinde daha modern bir görünüm kazandırıldı
    + önceden kaydettiğin kelimeleri bulmanı sağlayan sistem getirildi
    + önceden kaydettiğin kelimeleri değiştirmeni sağlayan sistem getirildi
    + kelime değiştirmedeki tekli sisteme çoklu sistem eklendi
    + komutları tamamlama sistemi eklendi
    + komut yazdıktan sonra komutla ilgili bilgi veren bölüm eklendi
    + 
    18.10.2021 Pazartesi 00:07
                    ''', assets = False
                    )
                ...
                try: 
                    ...
                    self.console_write(
                        'İnternet bağlantısında bir problem var, kontrol et dostum.', 
                        assets = False
                        )
                else:
                ...
                if name:
                    try:
                        ...
                        self.console_write("'{}' bulunamadı.".format(name), assets = False)
                    else:
                        ...
                        if PATH:
                            ...
                            self.console_write("'{}', '{}' konumunda dışarıya aktarıldı.".format(name, PATH), assets = False)
                            ...
                            self.console_write('Dışarı aktarma işlemi iptal edildi.', assets = False)

                        del _type
                else:
                    self.console_write('Lütfen dışarıya aktarmak istediğiniz kelime grubunun ismini yazınız. (Örn. dışa aktar {})'.format(list(self.words.keys())[random.randrange(0, len(self.words.keys()))]), assets = False)
                ...
                try:
                    with open(PATH, 'r', encoding = 'utf-8') as file:
                        ...
                        text = '{} tane kelime eklendi.'.format(len(words) - errors.count('\n'))
                    ...
                    self.console_write('Kelimeleri içeri aktarabilmek için bir dosya seçmeniz gerekiyor.', assets = False)
                del PATH
            else:
                self.console_write('GEÇERSİZ KOMUT!', assets = False)
...
class Ui_Dialog(object):
    ...
    def retranslateUi(self):
        _translate = QtCore.QCoreApplication.translate
        self.Dialog_.setWindowTitle(_translate('Dialog', 'Ayarlar'))
        self.groupBox.setWhatsThis(_translate('Dialog', 'Pencere rengini özelleştirmeni sağlar.'))
        self.groupBox.setTitle(_translate('Dialog', 'Renk'))
        self.toolButton.setText(_translate('Dialog', 'Seç'))
        self.groupBox_2.setWhatsThis(_translate('Dialog', 'Pencere altında yanıp sönen flaşörün rengini özelleştirmeni sağlar.'))
        self.groupBox_2.setTitle(_translate('Dialog', 'Flaşör Rengi'))
        self.toolButton_2.setText(_translate('Dialog', 'Seç'))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab), _translate('Dialog', 'Pencere'))
        self.groupBox_4.setWhatsThis(_translate('Dialog', 'Yazı tipi rengini özelleştirmeni sağlar.'))
        self.groupBox_4.setTitle(_translate('Dialog', 'Renk'))
        self.toolButton_4.setText(_translate('Dialog', 'Seç'))
        self.groupBox_3.setWhatsThis(_translate('Dialog', 'Fontu özelleştirmeni sağlar.'))
        self.groupBox_3.setTitle(_translate('Dialog', 'Font'))
        self.label.setText(_translate('Dialog', self.font_))
        self.toolButton_3.setText(_translate('Dialog', 'Seç'))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_2), _translate('Dialog', 'Yazı Tipi'))
        self.groupBox_5.setWhatsThis(_translate('Dialog', 'Sesi ayarlar.'))
        self.groupBox_5.setTitle(_translate('Dialog', 'Ses Seviyesi'))
        self.groupBox_6.setWhatsThis(_translate('Dialog', 'Konsolda yazarken çıkacak sesi özelleştirmeni sağlar.'))
        self.groupBox_6.setTitle(_translate('Dialog', 'Yazma Sesi'))
        self.label_2.setText(_translate('Dialog', '#sound.mp3'))
        self.toolButton_5.setText(_translate('Dialog', 'Seç'))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_3), _translate('Dialog', 'Ses'))
        self.groupBox_7.setWhatsThis(_translate('Dialog', 'Kelime eklerken kelimeleri ayıran karakteri değiştirmeni sağlar.'))
        self.groupBox_7.setTitle(_translate('Dialog', 'Kelime Ayracı'))
        self.radioButton.setText(_translate('Dialog', ';'))
        self.radioButton_2.setText(_translate('Dialog', ':'))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_4), _translate('Dialog', 'Diğer'))
...
class QLineEdit_(QtWidgets.QLineEdit):
    ...
    def dropEvent(self, event):
        ...
        if self.window.pushButton.text() == 'Değiştir':
            ...
            self.window.label.setText('Değiştir? (e/h)')
            self.window.pushButton.setText('↑')
        else:
            self.setText('içe aktar {}'.format(text))
        event.acceptProposedAction()
